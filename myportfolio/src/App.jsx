...
function CountUp({ value }) {
  ...
  useEffect(() => {
    if (!shown || reduced() || !m) { setDisplay(value); return; }
...
const STATS = [
  { n: "2+", l: "years shipping React" },
  { n: "3", l: "companies, incl. remote" },
...
const EXPERIENCE = [
  ...
  },
  {
    ...
    blurb: "My first role — and where full-stack clicked for me.",
    points: [
...
function CodeCard() {
  ...
  useEffect(() => {
    if (!shown) return;
    if (reduced()) { setCount(CODE_LINES.length); return; }
    let i = 0;
      ...
      <div className="hero-grid" style={{ position: "relative", width: "100%", maxWidth: 1240, margin: "0 auto",
        ...
        <div>
          ...
          <p className="rise" style={{ animationDelay: "260ms", fontFamily: SANS, fontSize: "clamp(16px,2vw,19px)", lineHeight: 1.7,
            ...
            Frontend engineer, 2+ years in. I care about the boring parts users never notice — load times, re-renders, the button that has to work on every screen. That’s the stuff that makes an interface feel good.
          </p>
      ...
      <div className="about-grid" style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: "clamp(32px,5vw,64px)", alignItems: "start" }}>
        ...
        <div>
          <div style={{ fontFamily: SERIF, fontSize: "clamp(22px,2.8vw,30px)", lineHeight: 1.55, color: C.ink, fontWeight: 400 }}>
            ...
            <p style={{ margin: "0 0 26px" }}>
              <StoryLine text="That curiosity turned into an MCA, then into real work: three companies, two countries, and a habit of asking what a slow page is hiding." delay={60} />
            </p>
            <p style={{ margin: 0 }}>
              <StoryLine text="Two years later, I’m still chasing the same thing — interfaces that are quick, accessible, and quietly reliable. The kind you don’t think about, because they never get in your way." delay={120} />
            </p>
          </div>
...
function Work() {
  return (
    <section id="work" style={{ padding: "80px clamp(20px,5vw,72px)", maxWidth: 900, margin: "0 auto", scrollMarginTop: NAV_OFFSET }}>
      <SectionHead label="01 — where I’ve worked" title="Two years, three teams" note="Each stop taught me something the last one couldn’t. Here’s the short version." />
